...
- [x] `GET /api/agent_info` — complete as of 14/8/2026.
  - [x] `prompt_examples[]` — three, captured verbatim from production runs against `dpl_8GYbarzwkSkewxayPA9Su4d8t2d4` (commit `904b63e`) and generated into `lib/prompt_examples.py`: the refinement gate answering in one call, a full disruption running all four modules, and — on the same conversation — a follow-up ("are meals included at that hotel?") that narrows to `DocumentationAgent` alone, showing both the narrowing and the 9/8 split that sends terms questions to the Contract of Carriage rather than to the agent that proposed the option. Between them every module name on the architecture PNG appears in a real trace. **Re-capture whenever a system prompt or the dispatch logic changes** — `tests/test_execute.py` checks their shape, not that they still match the code.
  - [x] `description` states decision D2: flights and stays are real, prices/availability/fare conditions are estimates. Also states the partial-failure posture (§5a), so the description and the behaviour agree.
...
- [x] Timing check: measured in production 14/8/2026 — **63.9s** for a full turn (7 LLM calls, all four modules, live AeroDataBox + Overpass + Pinecone), 48.7s for a follow-up that runs the reflection loop, 7.0s for one that only re-runs `FlightAgent`, 2.1s for the refinement gate. Well inside the 300s limit; the reflection loop is the cost centre in every case.
- [ ] Module-name consistency pass: diagram vs. `steps` vs. `agent_info` text
- [x] **GitHub repo is public** — verified 14/8/2026 with an unauthenticated `api.github.com/repos/anna-kravets/wingman-agent`, which returns `"private": false`.
- [ ] Submit — exact format:
  ```
  Vercel URL: {url}
  GitHub Repo URL: {url}
...
| 14/8/2026 | `prompt_examples[]` ships as a **generated Python module** (`lib/prompt_examples.py`), captured from production, not hand-written and not a JSON data file | Same reason airport coordinates are a module (9/8/2026): Vercel's Python builder traces imports, so a `.json` needs an `includeFiles` glob and fails in production only. Hand-writing was never an option — the spec's worked example carries every step verbatim, and a grader can diff them against a live run. The cost is a 537KB source file, most of it DocumentationAgent's retrieved passages; that is what an honest RAG trace weighs. |
