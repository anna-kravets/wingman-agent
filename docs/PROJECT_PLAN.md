...
| 15/8/2026 | Conflicts in `BLOCKING_CONFLICTS` (`route`, `stranded_at`) stop the dispatch and are asked about; the rest — including `incident_time` — become stated `assumptions`, and impossible fields are cleared | A route that cannot exist or an airport nobody can look up leaves the crew nothing to search against, so it is worth a round trip. Bouncing a stressed passenger over a cosmetic mismatch is the friction this exists to remove. **Reversal (same day):** `incident_time` was in `BLOCKING_CONFLICTS` too, on the theory that a wrong date corrupts the flight window and the entitlement clock — but the flight window comes from `local_now`, not `incident_time`, whose only consumer anywhere is one informational line in `documentation_agent.py`. Blocking it meant a disruption reported weeks late — the product's own core "compensation discovered too late" case — got asked a question with no right answer, because the date was right. It is now soft like any other conflict: stated as an assumption and cleared. |
...
| 15/8/2026 | A `WINGMAN_ALLOW_LLM=0` guard, checked at **call time** in `lib.llm._config` and `lib.rag.retrieve._production_retriever` (mirroring `lib.tools.live_data_enabled`), lets an ad-hoc script or REPL session opt itself out of spending the $13 budget | `api/index.py`'s `load_dotenv()` sets any env var not already present, so a variable a caller *deleted* before importing `api.index` is silently re-armed from the real `.env`. `tests/conftest.py`'s autouse fixture protects the pytest suite only — this already cost real budget once outside it. Checking at call time, not at import time, means `load_dotenv()` re-arming `LLMOD_API_KEY`/`PINECONE_API_KEY` cannot undo the opt-out. |
